...
void Neutral_Stake::neutral_stake_control() {
    if (master.get_digital(DIGITAL_B)) {
        if (neutral_stake_rot.get_angle() <= 13500 or neutral_stake_rot.get_angle() >= 34000) {
    ...
    if (master.get_digital(DIGITAL_A)) {
        while (neutral_stake_rot.get_angle() <= 2200 or neutral_stake_rot.get_angle() >= 34000) {
            neutral_stake_mtr.move_velocity(600);
        } if (neutral_stake_rot.get_angle() >= 4000) {
            while (neutral_stake_rot.get_angle() >= 3200) {
                neutral_stake_mtr.move_velocity(-600);
            } 
        }
        neutral_stake_mtr.move_velocity(0);
...
void Neutral_Stake::move1() {
    while (neutral_stake_rot.get_angle() <= 13500) {
        neutral_stake_mtr.move_velocity(500);
    } neutral_stake_mtr.move_velocity(0);
}
...
void Neutral_Stake::set_up() {
    while (neutral_stake_rot.get_angle() <= 2500 or neutral_stake_rot.get_angle() >= 34000) {
        neutral_stake_mtr.move_velocity(600);
    }
    while (neutral_stake_rot.get_angle() >= 2900) {
        neutral_stake_mtr.move_velocity(-600);
    } 
    neutral_stake_mtr.move_velocity(0);
    set_brake_mode('H');
